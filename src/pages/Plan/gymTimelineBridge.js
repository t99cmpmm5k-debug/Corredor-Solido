...
import { getTodayGymDay } from "../Gym/gymSchedule.js";

// { routine, day } del día de gimnasio programado (day.weekday) para el
// día de la semana de `iso` -- null si ninguna rutina tiene un día
// programado ese día. getTodayGymDay() ya aplica el mismo dedupe por id
// que "Próximos entrenamientos" (ver scheduledDays() en gymSchedule.js);
// con dos rutinas distintas para el mismo día de la semana, gana la
// primera por orden de getRoutines(), igual que ahí.
export function getGymDayForDate(iso) {
    ...
    return { routine: { id: match.__routineId, name: match.__routineName }, day: match };
