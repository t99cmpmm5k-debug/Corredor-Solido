...
- `src/pages/Carreras/` — calendario mensual de carreras (`workouts` con `type: "race"`, filtrados de `workoutStore.js`). `carrerasStore.js` guarda el mes visto y el día seleccionado; `initCarrerasEvents.js` abre el detalle de una carrera saltando a Running (`navigate(Running)` + `openDetail()` de `initRunningEvents.js`) — mismo patrón que `viewSessionWorkout()` en `Plan/initPlanEvents.js`, no una vista de detalle propia.
