...
- `POST /api/community/entrenos/:id/comments` (mismo header) — `{ text }` → `201 { id, alias, text, createdAt, isMine: true }` (Fase 3c). Comenta el entreno `:id`, cualquiera de cualquier usuario. Validación: `text` no vacío tras `trim()`, máximo 500 caracteres — `400 { error }` si no cumple, nunca guarda nada a medias. Texto guardado y devuelto tal cual, sin escapar HTML aquí — el escape es responsabilidad del frontend al insertarlo en el DOM, mismo criterio ya usado para `alias_publico`.
- `GET /api/community/entrenos/:id/comments` (mismo header) — `{ comments: [{ id, alias, text, createdAt, isMine }] }`, orden cronológico ascendente (el más antiguo primero, como cualquier hilo). `isMine` compara por `user_id` real, nunca por alias (`alias_publico` no es única — dos usuarios podrían compartir uno).
- `DELETE /api/community/entrenos/:id/comments/:commentId` (mismo header) — borra un comentario. `404 { error }` si ese `commentId` ya no existe; `403 { error }` si existe pero no es tuyo (ni siquiera si eres el dueño del entreno comentado) — solo su propio autor puede borrarlo. `{ deleted: true }` si es el tuyo.
